...
{
    if (_owner->GetSpawnId() == 43580 || _owner->GetSpawnId() == 43581)
        TC_LOG_DEBUG("random", "MotionMaster::InitializeDefault: Adding Factory Selector for {}", _owner->GetSpawnId());
...
void MotionMaster::MoveRandom(float wanderDistance)
{
    ...
    {
        if (_owner->GetSpawnId() == 43580 || _owner->GetSpawnId() == 43581)
            TC_LOG_DEBUG("random", "MotionMaster::MoveRandom: _owner: {} wanderDistance: {}", _owner->GetSpawnId(), wanderDistance);
